...
async function fetchTechInvoices(): Promise<AdminTechInvoiceRow[]> {
  ...
  return (data ?? []) as unknown as AdminTechInvoiceRow[];
}
...
export default function AdminInvoicesPage() {
  ...
  const totals = React.useMemo(() => {
    ...
    const open = invoices.filter(
      (inv) => (inv.status ?? "").toLowerCase() !== "paid" && inv.status !== null
    );

    const sum = (rows: AdminTechInvoiceRow[]) =>
      rows.reduce((acc, inv) => acc + (inv.total_cents ?? 0), 0);

    const sumPaid = sum(paid);
    const sumOpen = sum(open);

    const allTotals = invoices.map((inv) => inv.total_cents ?? 0);
    const avg =
      allTotals.length > 0
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                // Placeholder for future full CSV export
              ...
              <div className="divide-y divide-slate-800/80">
                {filteredInvoices.map((inv) => {
                  ...
                  const pdfInvoice: AnyObj = {
                    ...
                    tip_amount: 0,
                    total_amount: (inv.total_cents ?? 0) / 100,
                    ...
                    >
                      ...
                      <div className="hidden md:grid grid-cols-[1.3fr_1.5fr_0.9fr_0.8fr_0.7fr_0.7fr] gap-3 items-center">
                        ...
                        <div className="flex flex-col gap-1 text-xs">
                          ...
                          <p className="text-[11px] text-slate-500">
                            Subtotal ${centsToDollars(inv.subtotal_cents)} · Tax $
                            {centsToDollars(inv.tax_cents)}
                          </p>
                        </div>
