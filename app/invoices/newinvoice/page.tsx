...
type TechInvoice = {
  ...
  // money fields expected by ServicesPerformed
...
// keep your existing unique invoice number behavior
...
export default function TechNewInvoicePage() {
  ...
  // 🔥 IMPORTANT FIX:
  // We DO NOT create a row on mount anymore.
  // We keep local draft fields until the tech clicks "Start Draft".
  ...
  // snapshot of totals from <ServicesPerformed>
  ...
  // Load tech session (NO DB WRITES)
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      ...
      if (!email) {
        router.replace(`/tech/login?redirect=${encodeURIComponent("/tech/dashboard/invoices/newinvoice")}`);
      }
    })();
    return () => {
      mounted = false;
  ...
  /* --------- Load vehicles so tech can select user + vehicle --------- */
  const {
    data: vehicles,
    ...
    enabled: authReady, // only after auth attempt
    queryFn: async () => {
  ...
  const customerOptions = React.useMemo(() => {
    ...
    return Array.from(map.entries()).map(([email, vs]) => ({ email, vehicles: vs }));
  }, [vehicles]);

  const vehiclesForCustomer = React.useMemo(() => {
    if (!selectedCustomerEmail) return [];
    return customerOptions.find((c) => c.email === selectedCustomerEmail)?.vehicles ?? [];
  }, [customerOptions, selectedCustomerEmail]);

  /* --------- Create Draft Invoice (ONLY on button click) --------- */
  const createDraftMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload: any = {
        ...
        // attach anything already selected/typed BEFORE creating
        customer_email: selectedCustomerEmail || null,
  ...
  /* --------- Generic invoice meta updater (ONLY if invoice exists) --------- */
  const updateMetaMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // re-fetch the latest row (keeps UI consistent with ServicesPerformed/Map edits)
      const { data, error } = await supabaseClient
        .from("tech_invoices")
        .select(
          "id, invoice_number, services_json, windshield_repairs_json, technician_email, vehicle_id, invoice_date, status, customer_email, service_address, subtotal_cents, discount_percent, discount_cents, tax_rate_percent, tax_cents, total_cents, appointment_snapshot"
  ...
  const handleCustomerChange = (email: string) => {
    ...
    // If invoice already exists, update DB immediately
    if (invoice?.id) {
  ...
  const handleBasicMetaSave = () => {
    // If invoice does not exist yet, user should click "Start Draft" first.
    if (!invoice?.id) return;
  ...
  /* --------- Capture totals from ServicesPerformed (pricing) --------- */
  ...
  /* --------- Send Invoice (update tech_invoices) --------- */
  ...
  /* --------- Mark Paid (update tech_invoices only) --------- */
  ...
  // Auth check loading screen (NO inserts)
  if (!authReady) {
    return (
      ...
      <div className="max-w-5xl mx-auto space-y-6 relative z-10">
        ...
        <div className="flex items-center justify-between mb-2 print:hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* ✅ This is the key fix: user must click to create the row */}
            {!draftExists && (
              <Button
        ...
        <Card className="border border-slate-700/80 bg-gradient-to-br from-slate-900/80 via-slate-900/60 to-slate-950/90 backdrop-blur-xl shadow-[0_28px_80px_rgba(15,23,42,0.9)] print:bg-white print:border-slate-200 print:shadow-none">
          <CardContent className="p-6 md:p-8">
            <div className="grid md:grid-cols-[1.8fr_1.4fr] gap-8 items-start">
              {/* Business identity + mini 3D hero */}
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-[0.7rem] tracking-[0.25em] uppercase text-cyan-200/80">Glass Guardian</p>
                    <h1 className="text-2xl md:text-3xl font-extrabold text-slate-50 leading-tight">
                      Chip &amp; Crack Repair
                    </h1>
                    <p className="text-xs text-slate-400">
                      Mobile windshield specialists · Rock chip &amp; crack stabilization · Lifetime craftsmanship on
                      repairs
                    </p>
                  </div>
                </div>
                  ...
                  <div className="rounded-xl border border-amber-400/40 bg-amber-500/10 p-3 text-xs text-amber-100">
                    <div className="flex items-start gap-2">
                      ...
                      <p>
                        ...
                        want, then click <span className="font-semibold">Start Draft</span> to create the row in
                        Supabase.
                      </p>
                    </div>
                  </div>
              ...
              <div className="md:text-right space-y-4">
                <div className="inline-flex md:flex md:flex-col items-start md:items-end gap-2">
                  <p className="text-[0.65rem] font-semibold text-slate-400 tracking-[0.22em] uppercase">New Invoice</p>
                    ...
                    <span>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (invoice?.id) {
                            updateMetaMutation.mutate({ invoice_date: e.target.value || null });
                          }
                        }}
                ...
                <div className="space-y-1 text-xs md:text-sm text-slate-200 print:text-slate-800">
                  ...
                  <p className="font-semibold">{techEmail || invoice?.technician_email || "Technician"}</p>
                  <p className="text-slate-400 text-xs">Build a standalone invoice for any Glass Guardian customer.</p>
                </div>

                {vehiclesErr && (
                  <p className="mt-2 text-[11px] text-amber-300">
                    Note: vehicles failed to load. You can still create a manual invoice.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
                ...
                >
                  ...
                  {customerOptions.map((c) => (
                    <option key={c.email} value={c.email}>
                      {c.email}
                      {c.vehicles[0]
                        ? ` · ${c.vehicles[0].year ?? ""} ${c.vehicles[0].make ?? ""} ${c.vehicles[0].model ?? ""}`
                        : ""}
                    </option>
                  ))}
                </select>
                {loadingVehicles && <p className="text-[11px] text-slate-400 mt-1">Loading vehicles…</p>}
              </div>
          ...
          <Card className="border border-slate-700/70 bg-slate-900/70 backdrop-blur-xl shadow-[0_18px_60px_rgba(15,23,42,0.8)] print:bg-white print:border-slate-200 print:shadow-none">
            ...
            <CardContent className="space-y-3 text-sm text-slate-100 print:text-slate-800">
              <div className="space-y-1">
                <label className="text-xs uppercase tracking-[0.18em] text-slate-400">Vehicle (filtered by customer)</label>
                <select
                  className="w-full h-9 rounded-md bg-slate-900/70 border border-slate-600 text-xs text-slate-100 px-2"
                ...
                >
                  {!selectedCustomerEmail && <option value="">Select customer first</option>}
        ...
        {/* 🔹 Services Performed (requires invoice id) */}
        {effectiveInvoice ? (
          <ServicesPerformed
            invoice={{
        ...
        {/* 🔹 Windshield Repair Map (requires invoice id) */}
        {effectiveInvoice ? (
        ...
        <Card className="border border-emerald-500/50 bg-gradient-to-br from-emerald-500/15 via-slate-900/80 to-emerald-700/30 backdrop-blur-2xl shadow-[0_24px_80px_rgba(16,185,129,0.7)] print:bg-white print:border-emerald-300 print:shadow-none">
          ...
          <CardContent className="space-y-2 text-xs md:text-sm text-emerald-50 print:text-emerald-900">
            ...
            <ul className="list-disc ml-5 space-y-1">
              <li>
                <span className="font-semibold">Coverage:</span> 1 year from the service date for the repaired damage
                only.
              </li>
