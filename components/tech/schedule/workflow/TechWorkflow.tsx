...
export const WORKFLOW_STEPS = [
  ...
  { id: "inspect", label: "Inspect Damage", status: "on_site", icon: AlertCircle },
  ...
  { id: "complete", label: "Complete & Final Check", status: "completed", icon: FileText },
] as const;
...
export function statusToWorkflowStep(status?: string | null): number {
  ...
  if (status === "completed") return WORKFLOW_STEPS.length - 1;
  return 0;
}
...
export default function TechWorkflow(props: {
  ...
  const clampedStep = clampWorkflowStep(currentStep);

  const [confirmOpen, setConfirmOpen] = React.useState(false);
  const [certifiedComplete, setCertifiedComplete] = React.useState(false);
  const [certifiedCleaned, setCertifiedCleaned] = React.useState(false);
  const [certifiedDocumented, setCertifiedDocumented] = React.useState(false);
  ...
  const finalChecklistReady =
    certifiedComplete && certifiedCleaned && certifiedDocumented;

  const resetConfirmation = React.useCallback(() => {
    setCertifiedComplete(false);
    setCertifiedCleaned(false);
    setCertifiedDocumented(false);
  }, []);

  const openFinalConfirmation = React.useCallback(() => {
    resetConfirmation();
    setConfirmOpen(true);
  }, [resetConfirmation]);

  const closeFinalConfirmation = React.useCallback(() => {
    setConfirmOpen(false);
    resetConfirmation();
  }, [resetConfirmation]);

  const handleConfirmComplete = () => {
    if (!finalChecklistReady || !canComplete || completing || workflowLocked) return;
    completeJobAction();
    setConfirmOpen(false);
  };
          ...
          >
            <div className="flex min-h-full items-center justify-center p-4">
              <motion.div
                ...
                className="w-full max-w-xl rounded-3xl border border-slate-700 bg-slate-950 shadow-2xl overflow-hidden"
              >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Confirm the repair is fully complete before closing the job.
                      </p>
                    </div>
                ...
                <div className="p-5 space-y-4">
                  <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Confirm each item below to complete the job.
                    </p>
                    ...
                    <div className="mt-4 space-y-3">
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={certifiedComplete}
                          onChange={(e) => setCertifiedComplete(e.target.checked)}
                      ...
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={certifiedCleaned}
                          onChange={(e) => setCertifiedCleaned(e.target.checked)}
                          className="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-900 text-sky-400"
                        />
                        <span className="text-sm text-slate-200">
                          The work area and glass were cleaned properly.
                        </span>
                      </label>
                      ...
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={certifiedDocumented}
                          onChange={(e) => setCertifiedDocumented(e.target.checked)}
                          className="mt-1 h-4 w-4 rounded border-slate-600 bg-slate-900 text-sky-400"
                        />
                        <span className="text-sm text-slate-200">
                          Notes, photos, and any crack-out details were documented correctly.
                        </span>
                      </label>
                    </div>
                  </div>
                  ...
                  >
                    {isCrackOut
                      ? "This job will complete as a crack-out and route with replacement required."
                      : "This job will complete as a finished repair."}
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      disabled={
                        !finalChecklistReady || !canComplete || completing || workflowLocked
                      }
                      className={cx(
                        "flex-1 text-slate-950 font-semibold",
                        isCrackOut
                  ...
                  {!canComplete && isCrackOut && (
                    <p className="text-xs text-amber-200/90">
                      Crack-out requires: cause + notes (min 10 chars) + photo.
                    </p>
                  )}
                </div>
              </motion.div>
            ...
            <CardTitle className="text-2xl flex items-center gap-3 text-slate-50">
              <div className="relative">
                ...
                <div className="relative w-12 h-12 bg-gradient-to-br from-sky-500 to-sky-600 rounded-xl flex items-center justify-center shadow-[0_0_18px_rgba(56,189,248,0.8)]">
                  <span className="text-white font-bold text-xl">{clampedStep + 1}</span>
                </div>
              </div>

              {WORKFLOW_STEPS[clampedStep]?.label ?? "Workflow"}
              ...
              >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-slate-300">
                          The customer must sign the waiver on device or you can mark
                          sign in portal.
                        </p>
                        <Button
                ...
                {clampedStep === 0 && (
                  <div className="space-y-4">
                    ...
                    >
                      {workflowLocked ? "Continue (Waiver Required)" : "Continue to Inspect"}
                ...
                {clampedStep === 1 && (
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-4 mt-2">
                        <div className="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center bg-slate-900/70 hover:bg-slate-900/90 transition-all">
                          <input
                            ...
                            capture="environment"
                            onChange={(e) =>
                              handlePhotoUploadAction(e, "before_repair_closeup", "before")
                            }
                            className="hidden"
                    ...
                    <div className="flex gap-3">
                      <Button
                        variant="outline"
                        onClick={() => setAndPersistStepAction(clampedStep - 1)}
                    ...
                    <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-amber-100">
                      <div className="flex gap-3 items-start text-sm">
                        ...
                        <div>
                          ...
                          <span>
                            If a crack-out occurs during repair, report it on the final step.
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        value={cureTime}
                        onChange={(e) =>
                          setCureTimeAction(parseInt(e.target.value || "0", 10))
                        }
                        min={15}
                        max={60}
                        className="!bg-slate-900/80 !border-slate-700 !text-slate-50 placeholder:!text-slate-500"
                    ...
                    <div className="rounded-xl border border-amber-500/60 bg-amber-500/10 px-4 py-3 text-amber-100">
                      <div className="flex gap-3 items-start text-sm">
                        ...
                        <div>
                          ...
                          <span>
                            Do not disturb the repair. Typical cure time is 30–45 minutes.
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-4 mt-2">
                        <div className="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center bg-slate-900/70 hover:bg-slate-900/90 transition-all">
                          <input
                            ...
                            capture="environment"
                            onChange={(e) =>
                              handlePhotoUploadAction(e, "after_repair_closeup", "after")
                            }
                            className="hidden"
                    ...
                    <div className="rounded-xl border border-slate-700/80 bg-slate-900/80 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1">
                            Choose “Crack-out occurred” only if the crack expanded during repair.
                          </p>
                        </div>
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                        <Button
                          ...
                          className={cx(
                            ...
                            repairOutcome === "completed" &&
                              "ring-2 ring-emerald-400/70 border-emerald-400/70"
                          )}
                          disabled={workflowLocked}
                        >
                        ...
                        <Button
                          ...
                          className={cx(
                            "border-slate-600 bg-slate-950/30 text-slate-50 hover:bg-slate-800 transition-all",
                            repairOutcome === "crack_out" &&
                              "ring-2 ring-amber-400/70 border-amber-400/70"
                          )}
                          disabled={workflowLocked}
                        >
                          ...
                          <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-amber-100">
                            <div className="flex gap-3 items-start text-sm">
                              ...
                              <div>
                                ...
                                <span>Add cause, notes, and a photo.</span>
                              </div>
                            </div>
                          </div>
                    ...
                    >
                      <div className="flex gap-3 items-start text-sm">
                        ...
                        <div>
                          <strong>{isCrackOut ? "Crack-out flagged." : "Ready for final check."}</strong>{" "}
                          <span>
                            Clicking below will open the final confirmation popup before the job completes.
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <Button
                        ...
                        className={cx(
                          "flex-1 text-slate-950 font-semibold transition-all",
                          isCrackOut
                            ? "bg-amber-500 hover:bg-amber-600"
                            : "bg-emerald-500 hover:bg-emerald-600"
                        )}
                ...
                {clampedStep === 5 && (
                  <div className="space-y-4">
                    <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-4 text-emerald-100">
                      <p className="text-sm font-semibold">Final check is handled from the previous step.</p>
                      <p className="text-xs text-emerald-200/80 mt-1">
                        Go back if you need to review the after photo or completion details.
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <Button
                        variant="outline"
                        onClick={() => setAndPersistStepAction(4)}
                        className="flex-1 border-slate-600 bg-slate-900/80 text-slate-50 hover:bg-slate-800 transition-all"
                      >
                        Back to Final Photos
                      </Button>
                    </div>
                  </div>
                )}
